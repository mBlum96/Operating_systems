...
def main():
    ...
    assert (fcntl.ioctl(g, GET_TYPE) == TYPE_NONE)

    # Set a type
    fcntl.ioctl(f, SET_TYPE, TYPE_PUB)
    fcntl.ioctl(g, SET_TYPE, TYPE_SUB)

    # Check we have a type
    assert (fcntl.ioctl(f, GET_TYPE) == TYPE_PUB)
    assert (fcntl.ioctl(g, GET_TYPE) == TYPE_SUB)

    # We should not be able to read
    try:
        ret_read = os.read(f, 100)
